...
class HideImageInImage:
    ...
    def encode(self, coverImageName, secretImageNameList ,keyFileName, newCoverImageName, rsIndex):
        self.binary_list = []
        self.d_binary = []
        self.l = 0
        self.x = 0
        ...
        for secretImageName in secretImageNameList:
            ...
            self.l = 0
            with open(keyFileName, "a") as keys:
                keys.write(f"{secretImage.shape[0]}\n{secretImage.shape[1]}\n")

        with open(keyFileName, "a") as keys:
            keys.write(f"{rsIndex}\n")
        cv2.imwrite(newCoverImageName, coverImage)
        print(f"Encoded image saved as {newCoverImageName}")
    ...
    def decode(self, coverImageName, keyFileName, userId):
        self.x = 0
        self.y = 0
        self.z = 0
        self.l = 0
        ...
        numberOfImages = int((len(key_list)-1)/2)
        ...
        for g in range(0, numberOfImages):
            ...
            self.bin(key1, key2)
            img = cv2.imread("D:/web_dev/stegApp/python_scripts/blackmod.png")
            img1 = img.copy()
            ...
            while(k < 3):
                for i in range(0, key1):
                    for j in range(0, key2):
                        ...
                        pos = pos+1
                k = k+1
            cv2.imwrite(f"D:/web_dev/stegApp/public/{userId}-decodedImage{g+1}.png", blackImage)
        self.resampleIndex = key_list.pop(0)
